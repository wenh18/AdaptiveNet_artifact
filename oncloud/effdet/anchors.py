...
class AnchorLabeler(object):
    ...
    def label_anchors(self, gt_boxes, gt_classes, filter_valid=True):
        ...
        for level in range(self.anchors.min_level, self.anchors.max_level + 1):
            ...
            cls_targets_out.append(cls_targets[count:count + steps].contiguous().view([feat_size[0], feat_size[1], -1]))
            box_targets_out.append(box_targets[count:count + steps].contiguous().view([feat_size[0], feat_size[1], -1]))
            count += steps
