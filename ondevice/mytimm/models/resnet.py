...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
        ...
        block_len = 16
        self.block_time = [0 for i in range(block_len)]
    ...
    def adjust_multiblocks_to_subnet(self, subnet,use_presubnet=False):
        '''
            used in testing subnets, do not use this method in training
        ...
        if use_presubnet:
            ...
            self.subnet = ''
            print('adjust_multiblocks_to_subnet use pre subnet:{}'.format(subnet))
                    
            for blockidx in range(len(self.subnet)):
                print(old_subnet[blockidx],self.subnet[blockidx])
                if old_subnet[blockidx] != self.subnet[blockidx]:
                    if self.subnet[blockidx] == 1:
                        tmp = self.get_blockchoice1(blockidx)
                    elif self.subnet[blockidx] == 2:
                        tmp = self.get_blockchoice2(blockidx)
                    elif self.subnet[blockidx] == 99:
                        tmp = torch.nn.identity()
                    self.blocksequence.__setitem__(blockidx,tmp)

        else:
            self.subnet = ''
            print('adjust_multiblocks_to_subnet:{}'.format(subnet))
            layers = []
            for blockidx in range(len(subnet)):
                if subnet[blockidx] != 99:
                    ...
                    layers.append(self.multiblocks_back[blockidx][0])
                else:
    ...
    def check_validated_feature(self):
        ...
        self.subnet_pre = global_var.get_value('subnet_pre')
        self.subnet_shared_block = global_var.get_value('subnet_shared_block') 
        ...
        if self.saved_feature.__contains__(self.subnet_shared_block):
            self.saved_feature = self.saved_feature[self.subnet_shared_block]
    ...
    def forward_features(self, x, subnet,batch_idx=-1,infer_type=2):
        if infer_type == 0:
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)
    ...
    def forward(self, x, subnet=None, expert_weights=None, validate_subnet=False,batch_idx=-1,infer_type=2):  # eg:expert_choice = [0, 1]
        if validate_subnet:
            x = self.forward_validate_subnet(x,batch_idx=batch_idx)
        elif subnet is not None:
            if not validate_subnet:
                x, features, feature_idxs = self.forward_features(x, subnet,batch_idx,infer_type)
